...
export default function Select({category,setSelectedcategory}:IProps) {
  console.log(category.imageURL);

  return (
    <Listbox value={category} onChange={setSelectedcategory}>
      ...
      <div className="relative ">
        <ListboxButton className="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-3 pr-2 pl-3 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-indigo-600 sm:text-sm/6">
          <span className="col-start-1 row-start-1 flex items-center gap-3 pr-6">
            <img alt="" src={category.imageURL} className="size-5 shrink-0 rounded-full bg-gray-100" />
          ...
          {categories.map((categoryItem) => (
            ...
            >
              <div className="flex items-center">
                <img alt="" src={categoryItem.imageURL} className="w-8 h-8 shrink-0 rounded-full" />
                <span className="ml-3 block truncate font-normal group-data-selected:font-semibold">{categoryItem.name}</span>
              </div>
